...
serve(async (req) => {
  ...
  try {
    ...
    // Cancel payment in Asaas using DELETE
    ...
    if (!response.ok) {
      ...
      console.error(`Erro ao cancelar no Asaas: HTTP ${response.status} - ${errBody}`);
      throw new Error(`Erro ao cancelar no Asaas: HTTP ${response.status}`);
    }
    ...
    if (updateError) {
      console.error('Fatura cancelada no Asaas mas falhou ao atualizar localmente:', updateError);
      throw new Error('Cancelada no Asaas, mas falhou ao atualizar no sistema');
    }

    console.log(`Fatura ${fatura.numero_fatura} cancelada no Asaas e localmente`);
